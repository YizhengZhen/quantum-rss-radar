...
def load_papers_from_jsonl(path: Path, sample: int = 0):
    ...
    papers_with_abstract = [p for p in papers_raw if p.get("abstract") and len(p["abstract"]) > 50]
    ...
    if sample and sample < len(papers_with_abstract):
        ...
        sampled = [papers_with_abstract[i] for i in range(0, len(papers_with_abstract), step)][:n]
    else:
...
def format_comparison(old_score, old_dir, new_score, new_dir, title):
    ...
    delta = new_score - old_score if old_score is not None and new_score is not None else 0
    old_str = f"{old_score:>4.1f}" if old_score is not None else " N/A"
    new_str = f"{new_score:>4.1f}" if new_score is not None else " N/A"
    delta_str = f"{delta:+4.1f}" if old_score is not None and new_score is not None else "  N/A"
...
def main():
    ...
    parser.add_argument("--sample", default="20", help="Number of papers to analyze. 'all' for all. (default: 20)")
    parser.add_argument("--dry-run", action="store_true", help="Only show stats, no API calls")
    ...
    if args.dry_run:
        ...
        old_directions = Counter(p.get("direction") or "General / Other" for p in papers_raw)
        ...
        print(f"    uv run python scripts/rerun_analysis.py {jsonl_path.name} --sample 20")
        return
    ...
    if old_clean and new_scores:
        ...
        print(f"    {'Mean':>20} {old_mean:>8.2f} {new_mean:>8.2f} {new_mean-old_mean:>+8.2f}")
        old_med = sorted(old_clean)[len(old_clean)//2]
        new_med = sorted(new_scores)[len(new_scores)//2]
        print(f"    {'Median':>20} {old_med:>8.1f} {new_med:>8.1f} {new_med-old_med:>+8.1f}")
        print(f"    {'Min':>20} {min(old_clean):>8.1f} {min(new_scores):>8.1f} {min(new_scores)-min(old_clean):>+8.1f}")
        print(f"    {'Max':>20} {max(old_clean):>8.1f} {max(new_scores):>8.1f} {max(new_scores)-max(old_clean):>+8.1f}")
        ...
        for lo in range(0, 10):
            ...
            print(f"    [{lo}-{lo+1}) {old_c:>4}/{new_c:>4}  old:{bar_old:<20} new:{bar_new}")
    ...
    for paper, p_raw, analysis in sorted_by_delta[:5]:
        ...
        print(f"    {old_s:>4.1f} → {analysis.relevance_score:>4.1f} ({analysis.relevance_score - old_s:+>4.1f})  [{p_raw.get('direction','?')} → {analysis.direction}]")
        print(f"           {paper.title[:80]}")
    ...
    for paper, p_raw, analysis in comparisons:
        old_dir = p_raw.get("direction") or "General / Other"
        new_dir = analysis.direction or "General / Other"
        if old_dir != new_dir and new_dir != "General / Other" and p_raw.get("score", 0) < 5:
    ...
    if dir_changes:
        ...
        for p_raw, paper, analysis in dir_changes[:5]:
            old_s = p_raw.get("score") or 0
            print(f"    {old_s:>4.1f} → {analysis.relevance_score:>4.1f}  [{p_raw.get('direction','?')} → {analysis.direction}]")
            print(f"           {paper.title[:80]}")
