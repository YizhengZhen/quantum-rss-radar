...
class QuantumRSSRadar:
    ...
    def run_pipeline(self, test_mode: bool = False):
        ...
        try:
            ...
            # Step 7: Filter and rank papers
            logger.info("Filtering and ranking papers...")
            papers_with_analyses = self.analyzer.filter_and_rank_papers(papers, analyses)
            
            if not papers_with_analyses:
                logger.warning("No papers passed the relevance filter")
                return False
            
            logger.info(f"Filtered to {len(papers_with_analyses)} relevant papers")
            
            # Step 8: Save outputs
            logger.info("Saving outputs...")
            
            # Save JSONL output
            save_jsonl_output(papers_with_analyses, self.config.output_dir)
            
            # Generate Markdown report
            generate_markdown_report(papers_with_analyses, self.categories, self.config.output_dir)
            
            # Step 9: Build website
            logger.info("Building website...")
            website_builder = WebsiteBuilder(self.config.web_dir)
            website_builder.build_website(papers_with_analyses, self.categories)
            
            # Step 10: Send email (if enabled)
            if self.config.email_enabled:
                logger.info("Sending daily email...")
                self._send_daily_email(papers_with_analyses)
            ...
            # Print summary
            self._print_summary(papers_with_analyses)
