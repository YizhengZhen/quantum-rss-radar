...
| 功能 | 状态 | 说明 |
|------|------|------|
| � **SQLite 持久化** | ✅ 已实现 | `data/radar.db` — 支持历史查询、跨天统计、方向统计 |
| 🗃️ **LLM 分析缓存** | ✅ 已实现 | `data/llm_cache.json` — 用 paper.id 为 key，跳过重复论文的 LLM 调用，节省 50-80% API 费用 |

## 9. �🔮 待实现 / 可扩展方向

- [ ] **Web UI 管理面板** — 可视化查看运行记录、手动触发 pipeline
- [ ] **论文收藏 / 忽略机制** — 基于历史反馈优化评分（依赖网页 UI）
- [ ] **Slack / Telegram 推送** — 替代或补充邮件
- [ ] **WebSocket 实时推送** — 新论文到达即时通知

---

*生成时间: 2026-06-04 | 由 Cline 协助生成*
